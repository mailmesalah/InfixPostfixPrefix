...
void getType(char equ[][50], int size);                      ///function to find the type of fix
int isBracketsBalanced(char equ[][50], int size);           ///function to balance the brackets
void infixToPostfix(char equ[][50], int size);             ///infix to postfix
void infixToPrefix(char equ[][50], int size);             ///infix to prefix
void postfixToInfix(char equ[][50], int size);           ///postfix to infix
void postfixToPrefix(char equ[][50], int size);         ///postfix to prefix
void prefixToPostfix(char equ[][50], int size);        ///prefix to postfix
void prefixToInfix(char equ[][50], int size);         ///prefix to infix
...
void print2DData(char equ[][50]);                            /// prints 2 dimensional character array
...
void fillBuffer(char strs[][50]);                   /// 2 dimensional char array is copied to outBuff char array
void printToFile(char* fileName, char* rw, char output[][50], char* param);/// prints outputs with parameters in the file specified by filename
void checkForError(char equ[][50], int size);      /// checks for any mismatch in operator operand count also checks for entry of invalid characters
int evaluateExp(char equ[][50], int size, int choice); // evaluates the expression
void printEValToFile(char* fileName, char* rw);// prints the evaluated output


///Global variables declaration
char equation[50][50];   ///char array of main equation
int eqSize;               ///array size
char postfix[10][50];  ///to store postfix  expression equation
char infix[10][50];   ///to store infix  expression equation
char prefix[10][50]; ///to store prefix  expression equation
...
int currentSize = 0; // current size of expression array
...
void push(char* data) {
	...
	struct node *temp;          /// creates a temporary node pointer variable
...
int main()
{
	...
	scanf("%d", &choice);   /// selects what user want to use keyboard input of file input
	...
	if (choice == 1) {
		...
		while ((tok = tokenizeStr(savePtr, " ", &savePtr)) != NULL) {/// iterates through each space delimited string
			copyStr(equation[totalElement++], tok);                 /// copies current token to equation string array
		}

		for (i = 0; i < totalElement; i++)      /// prints the tokens received in equation variable
			printf("%s ", equation[i]);

		eqSize = totalElement;            /// size of the equation array is stored in size.
		...
		if (choice == 1) {/// user selects for deafult file selection for input
			...
			eqSize = totalElement;/// number of tokens are kept in size
			...
			eqSize = totalElement;/// number of tokens are stored in size
...
void execute(int choice, char fileName[50])
{
	...
	printf("size %d\n", eqSize);  /// prints the size

	status = isBracketsBalanced(equation, eqSize);/// to see if parenthesis are balanced
	getType(equation, eqSize);/// checks the type of fix user inputed
	checkForError(equation, eqSize);/// checks for any operand operator mismatch also for invalid characters
	...
	if (status == 1)/// checks if parenthesis are balanced
	{
		...
		if (fType == 1 && convType == 1)/// if the type is infix and want to convert to prefix
		{
			...
			infixToPrefix(equation, eqSize);/// infix to prefix conversion function
			print2DData(prefix);/// prints the output

			total = evaluateExp(postfix, eqSize, fType); // evaluates
			printf("\nEvaluation Output : %d", total);// prints the value

			/// print to file
			if (choice == 1)/// prints to files input and output , inputs and outputs
			{
				/// prints to file
		...
		if (fType == 1 && convType == 2) /// infix to postfix conversion
		{
			...
			infixToPostfix(equation, eqSize);      ///function call to function which converts INFIX TO POSTFIX
		...
		if (fType == 1 && convType == 3)/// infix to both prefix and postfix conversions
		{
			...
			for (i = 0; i < eqSize; i++) /// copies equation in temporary variable to keep the data
			{
				copyStr(tempEqu[i], equation[i]);
			}

			printf("The postfix expression is :\n");
			infixToPostfix(equation, eqSize);                 ///function call which converts INFIX TO POSTFIX
		...
		if (fType == 2)        ///if type of input is a PREFIX
		{
			...
			if (convType == 1) {/// converts to infix
				...
				prefixToInfix(equation, eqSize);/// function call too prefix to infix
				print2DData(infix);/// prints the output

				total = evaluateExp(equation, eqSize, fType); // evaluates
				printf("\nEvaluation Output : %d", total);// prints the value
				...
				printf("The postfix expression is :\n");
				prefixToPostfix(equation, eqSize);/// prefix to postfix function call
				...
				/// copying to keep data
				for (i = 0; i < eqSize; i++) /// keeps equation in temp variable
				{
					copyStr(tempEqu[i], equation[i]);
				...
				printf("The infix expression is :\n");
				prefixToInfix(equation, eqSize);/// function call prefix to infix
				...
				printf("\n");
				printf("The postfix expression is :\n");
				prefixToPostfix(equation, eqSize);/// function call to convert prefix to postfix
		...
		if (fType == 3)                               ///if type of input is a POSTFIX
		{
			...
			if (convType == 1) {/// convert to infix
				printf("The Infix expression is :\n");
				postfixToInfix(equation, eqSize);/// function call to convert postfix to infix
				...
				printf("The prefix expression is :\n");
				postfixToPrefix(equation, eqSize);/// post fix to prefix conversion call
				print2DData( prefix);
				...
				/// copying to keep data
				for (i = 0; i < eqSize; i++)/// keeps the equation in temporary file
				{
					copyStr(tempEqu[i], equation[i]);
				...
				printf("The infix expression is :\n");
				postfixToInfix(equation, eqSize);/// function to convert from post fix to infix is called
				...
				printf("\n");
				printf("The prefix expression is :\n");
				postfixToPrefix(equation, eqSize);/// function to post fix to prefix is called
...
*/
void getType(char equ[][50], int size)   ///function to decide the type of fix
{
	int i = 0, j = 0, found = 1;
	...
	for (i = 0; i < size; i++)
	{
		if (equ[i][0] != '(')/// checks if the string is '('
		{
			...
			++i;
			/// check second operand to be a number
			if (isDigit(equ[i]))
			{
				found = 0;
				break;
	...
	/// prefix checking
	if (!isDigit(equ[0]))/// check for the character not a digit
	{
...
*/
int isBracketsBalanced(char equ[][50], int size)            /// function to check parenthesis balance
{
	int i = 0, j = 0;
	int a = 0, b = 0, c = 0;
...
void infixToPostfix(char equ[][50], int size)    /// function to convert infix to postfix with two arguments
												 /// infix equation and size of the equation as arguments
{
	...
	{
		//printf("\ni = %d j = %d equ = %s, stack = %s", i, j, equ[i], peek());
		...
		else if (compareStr(equ[i], '+') == 0 || compareStr(equ[i], '-') == 0 || compareStr(equ[i], '*') == 0 || compareStr(equ[i], '/') == 0 || compareStr(equ[i], '^') == 0)
		{
			...
			{
				push(equ[i]);           /// push current operator onto stack
			}
	...
	currentSize = size;//sets current size
	return;
}


void infixToPrefix(char equ[][50], int size)  /// to convert infix to prefix
{
	int i, j;
...
void postfixToInfix(char equ[][50], int size) /// postfix to prefix
{
	int i, j,l, len = 0;  /// len to keep the length of expression pushed to stack
	char temp[50],k[50];      /// temporary equ data holder
	...
	for (i = 0, j = 0; i < size; i++)
	{
		...
		else /// its operator
		{
			...
			{
				printf("\nError Stack empty Postfix to Infix");
				exit(1);
				/// its error
			...
			/// initialize temp
			temp[0] = '\0';
			strcat(temp, "( ");// startting paranthesis
			strcat(temp, peek());// first operand
			pop();
			strcat(temp, " "); // space
			strcat(temp, equ[i]);// operator
			strcat(temp, " ");// space
			strcat(temp, peek());// second operand
			pop();
			strcat(temp, " )");// closing parenthesis
			/// push back to stack
			push(temp);
		}
	}
	...
	//copy the output to infix

	k[0] = '\0';// null to initialise
	j = 0; l = 0;
	for (i = (strlen(temp)-1); i >=0; i--)// reverse copying temp data to infix
	{
		if (temp[i] == ' ')//check for space character
		{
			k[l] = '\0';// null terminator
			strcpy(infix[j++] ,strrev(k));// copies the token in reverse
			l = 0;//initialise l
			k[l] = '\0';// nulls the first character of k , initialising for new digit or operator
		}
		else {
			k[l++] = temp[i];// digit or operator is copied to k
		}
		
	}

	k[l] = '\0';
	strcpy(infix[j++], strrev(k));//last character is copied to infix

	currentSize = j;//sets current size
	//reverse all parathesis
	for (i = 0; i<currentSize; i++)
	{
		if (infix[i][0] == '(')
		{
			infix[i][0] = ')';//reverse parathesis
		}
		else if (infix[i][0] == ')')
		{
			infix[i][0] = '(';//reverse parathesis
...
void postfixToPrefix(char equ[][50], int size) /// postfix to prefix
{
	...
	for (i = 0, j = 0; i < size; i++)
	{
		...
		else /// its operator
		{
			if (isEmpty())
			{
				printf("\nError Stack empty Postfix to Prefix");
				exit(1);
				/// its error
				/// set failure global variable
				break;
	...
	k[0] = '\0';
	j = 0; l = 0;
	for (i = (strlen(temp) - 1); i >= 0; i--)//reverse copy to prefix variable
	{
		if (temp[i] == ' ')//check for space
		{
			k[l] = '\0';//null
			strcpy(prefix[j++], strrev(k));//copies the currently got token to prefix
			l = 0;
			k[l] = '\0';//null
		}
		else {
			k[l++] = temp[i];// if not pace keep that character in variable array
		}
	}

	k[l] = '\0';
	strcpy(prefix[j++], strrev(k));//last digit or operator

	currentSize = j;// keeps the size

}

void prefixToPostfix(char equ[][50], int size)

{
	...
	for (i = size - 1; i >= 0; i--)
	{
		...
		else /// its operator
		{
			...
			{
				printf("\nError Stack empty Prefix to Postfix");
				exit(1);
				/// its error
				/// set failure global variable
				break;
	...
	currentSize = 1;//sets current size
	strcpy(postfix[0], temp); // copies the output to prefix
	for (i = 1; i < size; i++)
	{
		postfix[i][0] = '\0';//resets all other values
	}
}

void prefixToInfix(char equ[][50], int size) /// converts prefix to infix

{
	...
	for (i = size - 1; i >= 0; i--)/// iterate from right to left
	{
		...
		else/// its operator
		{
			...
			{
				printf("\nError Stack empty Prefix to Infix");
				exit(1);
				/// its error
				/// set failure global variable
				break;
			...
			temp[0] = '\0';
			strcat(temp, "( ");// opening parenthesis
			strcat(temp, peek());// first operance
			pop();
			strcat(temp, " ");// space
			strcat(temp, equ[i]);// operator
			strcat(temp, " ");// space
			strcat(temp, peek());// second paranthesis
			pop();
			strcat(temp, " )");// closing paranthesis
	...
	currentSize = 1;//sets current size
	strcpy(infix[0], temp); // copies the output to prefix
	for (i = 1; i < size; i++)
	{
		infix[i][0] = '\0';//resets all other values
	}
...
int isDigit(char *str)
{
	...
	if (str[0] == '-') 
	{ // if number start with minus consider it as a number
		i = 1;
		dig = 0;// it can be an operator
		while (str[i] != '\0')/// iterates till the last character
		{
			dig = 1;// if there is data it can be a number
			//skip
...
*/
void print2DData(char equ[][50])
{
	int i;
	printf("\n");
	for (i = 0; i < currentSize; i++)/// iterates through the array
	{
...
{
	if (*str != c)/// checks first character of the string with character in c
	{
...
/*
the two dimensional data is saved to one dimensional outBuf
*/
void fillBuffer(char strs[][50])
{
	int i;
	outBuf[0] = '\0';/// delimits the buffer with null character
	for (i = 0; i < currentSize; i++)/// iterates through the 2D character array str
	{
		strcat(outBuf, strs[i]);/// each string is added to outBuff
		strcat(outBuf, " ");/// the string is separated by a space
	}
}
...
Prints data to file
*/
void printToFile(char* fileName, char* rw, char output[][50], char* param)
{
	/// Print to file
...
*/
void checkForError(char equ[][50], int size)
{
	...
	for (i = 0; i < size; i++)/// iterate through the expression
	{
		...
		if (compareStr(equ[i], '*')==0 || compareStr(equ[i], '/')==0 || compareStr(equ[i], '+')==0 || (compareStr(equ[i], '-')==0 && isdigit(equ[i][1])==0) || compareStr(equ[i], '^')==0)
		{
...
int evaluateExp(char equ[][50], int size, int choice)// evaluates the expressions
{
	...
	case 1:// input is infix
		...
		for (i = 0, j = 0; i < size; i++)//iterates throught the loop
		{
			...
			//k = 0;
			//printf("\n%d ", i);
			//for (k = 0; k < counter; k++)// loops to calculate power
			//{
			//	printf("%d ", stackedOperand[k]);
			//}
...
/*
Prints evaluated output to file
*/
